...
def user_to_evaluation_context(user: User) -> Dict[str, Any]:
    ...
    for group_type in user_groups:
        ...
        if isinstance(group_name, list) and len(group_name) > 0:
        ...
        if group_properties_type is None or isinstance(group_properties_type, dict):
            continue
        group_properties_name = group_properties_type[group_name]
        if group_properties_name is None or isinstance(group_properties_name, dict):
            continue
